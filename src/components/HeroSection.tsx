    ...
    <div className="relative overflow-hidden h-screen">
      ...
      <div className="container relative z-10 px-6 flex flex-col items-center justify-center h-full text-center">
        ...
        >
          ...
          >
            <Link to="/allprograms">
              <button className="px-8 py-4 bg-primary text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-transform hover:scale-105 w-full sm:w-auto">
                Explore All Programs
              </button>
            </Link>

            <Link to="/contact">
              <button className="px-8 py-4 bg-secondary text-secondary-foreground font-bold rounded-full shadow-lg hover:shadow-xl transition-transform hover:scale-105 w-full sm:w-auto">
                Enquire Now
              </button>
            </Link>
          </motion.div>
      ...
      {showPopup && (
        <div className="fixed z-50  max-w-2xl md:max-w-3xl lg:max-w-4xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            transition={{ duration: 0.5, ease: "easeOut" }}
            className="relative bg-foreground text-white shadow-2xl rounded-3xl p-10 border-l-4 border-green-800"
          >
            {/* Header with underline */}
            <div className="flex justify-between items-start mb-6">
              <div className="flex flex-col items-center w-full">
                <h4 className="font-serif text-white text-3xl md:text-4xl font-bold text-center">
            ...
            <Link
              to="/YCBcourse"
              className="inline-block w-full text-center bg-white text-foreground mt-3 font-bold rounded-full py-4 text-lg md:text-xl hover:bg-green-50 transition-all shadow-lg"
            >
              Explore Courses
            </Link>
          </motion.div>
